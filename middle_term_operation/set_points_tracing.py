    ...
    def set_points_tracing_ed(*args):
        ...
        try:
            for i in range(T):
                ...
                model["DG"]["COMMAND_START_UP"][i] = int(row.DG_STATUS)
                model["DG"]["COMMAND_PG"][i] = row.DG_PG

                model["UG"]["COMMAND_START_UP"][i] = int(row.UG_STATUS)
            ...
            # If any exception happens, it is not operated in set-point tracing method
