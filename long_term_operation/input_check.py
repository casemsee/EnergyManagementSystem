...
class InputCheck():
	def model_local_check(*args):
		...
		for i in range(T):
			if model["Load_nac"]["PD"][i] > model["Load_nac"]["STATUS"][i] * model["Load_nac"]["PMAX"]:
				...
				model["Load_nac"]["PD"][i] = model["Load_uac"]["STATUS"][i] * model["Load_nac"]["PMAX"]
		...
		if len(model["Load_ndc"]["STATUS"]) != T:
			...
			model["Load_ndc"]["STATUS"] = [configuration_default_load.default_Load_DC["STATUS"]] * T
	...
	def model_universal_check(*args):
		...
		model = InputCheck.model_local_check(model)
