...
def short_term_operation_uems(universal_mg, local_mg, socket_upload, socket_download, info, session, logger):
    ...
    thread_info_ex = SinglePeriodInformationUpdateThread(local_mg, info, socket_upload,
                                                         default_look_ahead_time_step["Look_ahead_time_opf_time_step"])
    ...
    universal_mg = SetPointsTracing.set_points_tracing_opf(Target_time, session,
                                                           universal_mg)  # There are some bugs in this function
    ...
    if local_mg["COMMAND_TYPE"] == 1 and universal_mg["COMMAND_TYPE"] == 1:
        ...
        mathematical_model = ProblemFormulationSetPointsTracing.problem_formulation_universal(local_mg, universal_mg,
                                                                                              "Feasible")
        mathematical_model_recovery = ProblemFormulationSetPointsTracing.problem_formulation_universal(local_mg,
                                                                                                       universal_mg,
                                                                                                       "Infeasible")
    else:
        ...
        mathematical_model_recovery = ProblemFormulation.problem_formulation_universal(local_mg, universal_mg,
                                                                                       "Infeasible")
    ...
    if res.value["success"] is True:
        ...
        (local_mg, universal_mg) = result_update(res_recovery.value, local_mg, universal_mg, "Infeasible",
                                                 mathematical_model_recovery)
    ...
    database_operation__uems = threading.Thread(target=database_storage_operation.database_record,
                                                args=(session, universal_mg, Target_time, "OPF"))
...
def short_term_operation(local_mg, session, session_history, session_real_time, logger):
    ...
    local_mg["COST"] = 0
    # Step 1: Short-term forecasting
    thread_forecasting = ShortTermForecastingThread(session, session_history, Target_time,
                                                    local_mg)  # The forecasting thread
    thread_forecasting.start()
    thread_forecasting.join()
    local_mg = thread_forecasting.microgrid
    ...
    local_mg = SetPointsTracing.set_points_tracing_opf(Target_time, session,
                                                       local_mg)  # Update the operation mode of local ems
...
def result_update_local(*args):
    ...
    local_model = update(x_local, local_model, type)
    local_model["COST"] = float(sum([c * d for c, d in zip(c_local, x_local)])) * default_time[
        "Time_step_opf"] / 3600  # Update cost
