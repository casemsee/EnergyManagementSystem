...
class ProblemFormulationSetPointsTracing():
    ...
    def problem_formulation_local(*args):
        ...
        from modelling.data.idx_opf_set_points_tracing import PG, QG, RG, PUG, QUG, RUG, PBIC_AC2DC, PBIC_DC2AC, \
            QBIC, PESS_C, \
        ...
        lb[PG] = model["DG"]["PMIN"] * min(model["DG"]["STATUS"], model["DG"]["COMMAND_START_UP"])
        lb[QG] = model["DG"]["QMIN"] * min(model["DG"]["STATUS"], model["DG"]["COMMAND_START_UP"])
        lb[RG] = model["DG"]["PMIN"] * min(model["DG"]["STATUS"], model["DG"]["COMMAND_START_UP"])

        lb[PUG] = model["UG"]["PMIN"] * min(model["UG"]["STATUS"], model["UG"]["COMMAND_START_UP"])
        lb[QUG] = model["UG"]["QMIN"] * min(model["UG"]["STATUS"], model["UG"]["COMMAND_START_UP"])
        lb[RUG] = model["UG"]["PMIN"] ** min(model["UG"]["STATUS"], model["UG"]["COMMAND_START_UP"])
        ...
        lb[SOC_negative] = 0
        ## Update lower boundary
        ub[PG] = model["DG"]["PMAX"] * min(model["DG"]["STATUS"], model["DG"]["COMMAND_START_UP"])
        ub[QG] = model["DG"]["QMAX"] * min(model["DG"]["STATUS"], model["DG"]["COMMAND_START_UP"])
        ub[RG] = model["DG"]["PMAX"] * min(model["DG"]["STATUS"], model["DG"]["COMMAND_START_UP"])

        ub[PUG] = model["UG"]["PMAX"] * min(model["UG"]["STATUS"], model["UG"]["COMMAND_START_UP"])
        ub[QUG] = model["UG"]["QMAX"] * min(model["UG"]["STATUS"], model["UG"]["COMMAND_START_UP"])
        ub[RUG] = model["UG"]["PMAX"] * min(model["UG"]["STATUS"], model["UG"]["COMMAND_START_UP"])
        ...
        c[PMG_positive] = default_eps["Penalty_opf"]
        c[PUG_positive] = default_eps["Penalty_opf"]
        c[PUG_negative] = default_eps["Penalty_opf"]
        c[SOC_positive] = default_eps["Penalty_opf"]
        c[SOC_negative] = default_eps["Penalty_opf"]
        c[PBIC_AC2DC] = default_eps[
                            "Penalty_opf"] / 10  # These two items are added to remove the bilinear constraints :1)
        c[PBIC_DC2AC] = default_eps["Penalty_opf"] / 10  # 2)
    ...
    def problem_formulation_local_recovery(*args):
        from configuration import configuration_time_line
        from modelling.data.idx_opf_set_points_tracing_recovery import PG, QG, RG, PUG, QUG, RUG, PBIC_AC2DC, \
            PBIC_DC2AC, QBIC, \
    ...
    def problem_formulation_universal(*args):
        ...
        if type == "Feasible":
            from modelling.data.idx_opf_set_points_tracing import PMG, NX
            local_model_mathematical = ProblemFormulationSetPointsTracing.problem_formulation_local(local_model)
            universal_model_mathematical = ProblemFormulationSetPointsTracing.problem_formulation_local(
                universal_model)
        else:
            from modelling.data.idx_opf_set_points_tracing_recovery import PMG, NX
            local_model_mathematical = ProblemFormulationSetPointsTracing.problem_formulation_local_recovery(
                local_model)
            universal_model_mathematical = ProblemFormulationSetPointsTracing.problem_formulation_local_recovery(
                universal_model)
