...
class OutputCheck():
    def output_local_check(*args):
        ...
        T = default_look_ahead_time_step[
            "Look_ahead_time_opf_time_step"]  # The look ahead time step of optimal power flow
        if model["success"] is True:
            if model["UG"]["COMMAND_PG"] + model["DG"]["COMMAND_PG"] - model["BIC"]["COMMAND_AC2DC"] + model["BIC"][
                "COMMAND_DC2AC"] * model["BIC"]["EFF_DC2AC"] - model["Load_ac"]["PD"] - model["Load_nac"]["PD"] >= \
                    default_eps[
                        "POWER_BALANCE"] or model["UG"]["COMMAND_PG"] + model["DG"]["COMMAND_PG"] - model["BIC"][
                "COMMAND_AC2DC"] + \
                    model["BIC"][
                        "COMMAND_DC2AC"] * model["BIC"]["EFF_DC2AC"] - model["Load_ac"]["PD"] - model["Load_nac"][
                "PD"] <= - \
                    default_eps[
                        "POWER_BALANCE"]:
                logger.error("The obtained solution can not meet AC bus power requirement!")
                logger.info(model["UG"]["COMMAND_PG"] + model["DG"]["COMMAND_PG"] - model["BIC"]["COMMAND_AC2DC"] +
                            model["BIC"]["COMMAND_DC2AC"] * model["BIC"]["EFF_DC2AC"] - model["Load_ac"]["PD"] -
                            model["Load_nac"]["PD"])
            ...
            if model["ESS"]["COMMAND_PG"] + model["BIC"]["COMMAND_AC2DC"] * model["BIC"]["EFF_DC2AC"] - \
                ...
                "PG"] + \
                    model["WP"]["PG"] - model["PMG"] >= default_eps["POWER_BALANCE"] or model["ESS"]["COMMAND_PG"] + \
                    model["BIC"][
                        "COMMAND_AC2DC"] * model["BIC"]["EFF_DC2AC"] - model["BIC"]["COMMAND_DC2AC"] - model["Load_dc"][
                "PD"] - \
                    model["Load_ndc"]["PD"] + model["PV"]["PG"] + model["WP"]["PG"] - model["PMG"] <= -default_eps[
                "POWER_BALANCE"]:
                ...
                logger.info(model["ESS"]["COMMAND_PG"] + model["BIC"]["COMMAND_AC2DC"] * model["BIC"]["EFF_DC2AC"] - \
                            model["BIC"]["COMMAND_DC2AC"] - model["Load_dc"]["PD"] - model["Load_ndc"]["PD"] +
                            model["PV"][
                                "PG"] + \
